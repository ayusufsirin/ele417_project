...
void hd44780_timer_isr(void)
{
    ...
    static const uint8_t u8__row_start_address[3] =
    {   0x40, HD44780_3RD_ROW_START_ADDRESS, HD44780_4TH_ROW_START_ADDRESS};
#endif
  ...
  if( !(u16__flags & 0x2000) )
  {
    ...
    else
    {
      ...
      #else
#if HD44780_NR_OF_ROWS < 2
        uint8_t u8__initialization_bytes[6] = { 0x30, 0x30, 0x30, 0x08, 0x06,
                                                0x0C };
#else
        uint8_t u8__initialization_bytes[6] =
        {   0x30, 0x30, 0x38, 0x08, 0x06, 0x0C};
#endif
#endif
        ...
        if (!(u16__flags & 0x007F))
        {
            ...
            {
                u8__data_byte =
                        u8__initialization_bytes[(u16__initialization_step - 2)];
                HD44780_ENABLE_HIGH;
            }
...
void hd44780_clear_row(uint8_t u8__row)
{
    ...
    #else
        for (u8__counter = 0; u8__counter < HD44780_NR_OF_COLUMNS;
                u8__counter++)
        {
            u8__hd44780_data_buffer[u8__counter] = ' ';
        }
...
void hd44780_blank_out_remaining_row(uint8_t u8__row, uint8_t u8__column)
{
    if (u8__row <= HD44780_NR_OF_ROWS)
    {
      ...
      #else
            for (u8__counter = (u8__column - 1);
                    u8__counter < HD44780_NR_OF_COLUMNS; u8__counter++)
            {
                u8__hd44780_data_buffer[u8__counter] = ' ';
            }
#endif
                                    ...
                                    uint8_t u8__cr_lf)
{
    ...
    if (u8__adc_resolution_bits == 8)
    {
        u16__max_adc_result = 255;
    }
    else if (u8__adc_resolution_bits == 10)
    {
        u16__max_adc_result = 1023;
    }
    else if (u8__adc_resolution_bits == 12)
    {
        u16__max_adc_result = 4095;
    }
    else if (u8__adc_resolution_bits == 14)
    {
        u16__max_adc_result = 16383;
    }
    else if (u8__adc_resolution_bits == 16)
    {
        u16__max_adc_result = 65535;
    }

    if (u16__max_adc_result)
    {
        u16__adc_value_mv = ((u16__adc_reference_mv * u32__adc_value)
                / u16__max_adc_result);
        u8__last_written_buffer_position = hd44780_output_unsigned_16bit_value(
                u16__adc_value_mv, u8__leading_zero_handling, u8__row,
                u8__column, u8__cr_lf);
    }
                                            ...
                                            uint8_t u8__cr_lf)
{
    ...
    if (u8__leading_zero_handling)
    {
        ...
        {
            u8__last_written_buffer_position = hd44780_write_string(
                    ch__string_buffer, u8__row, u8__column, u8__cr_lf);
        }
        else
        {
            u8__last_written_buffer_position = hd44780_write_string(
                    (ch__string_buffer + u8__counter), u8__row, u8__column,
                    u8__cr_lf);
        }
    }
    else
